...
class ProgressControl(models.Model):
    ...
    user_task_id = fields.Many2one(
        'user.task', 'Usuario Control', ondelete='cascade')
...
class ProgressControlLine(models.Model):
    ...
    state = fields.Selection(string='Estado', related='task_id.state')
    progress_control_id = fields.Many2one(
        'progress.control', 'Progress Control', ondelete='cascade')

    begin_date = fields.Datetime('Fecha Inicio')
    end_date = fields.Datetime('Fecha Fin')

    s_play = fields.Boolean('Iniciada')


    def start(self):
        for record in self:
            if not record.begin_date:
    ...
    def stop(self):
        for record in self:
            ...
            if record.begin_date:
                cant_horas = (record.end_date - record.begin_date).seconds / 3600

            record.cant_horas = cant_horas
