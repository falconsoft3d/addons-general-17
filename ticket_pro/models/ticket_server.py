...
FIELDS_MESSAGE = ['subject','date','body','model','message_type','email_from','reply_to']
...
class TicketServer(models.Model):
    ...
    transported_ticket_ids = fields.Many2many('ticket.pro')
    ...
    def create_remote_tickets(self, ticket):
        ...
        try:
            remote_tk = object_proxy.execute_kw(*record.credentials,
                                            'ticket.pro',
                                            'create', [values])
        except:
            pass
        try:
            ...
            for message in ticket.message_ids:
                message_values = {
                    att: getattr(message, att) for att in FIELDS_MESSAGE
                }
                message_values.update({
                                        'res_id': remote_tk,
                                       })
                message_list.append(message_values)
            message_list.reverse()
            for message in message_list:
                object_proxy.execute_kw(*record.credentials,
                                        'mail.message',
                                        'create', [message])
    ...
    def send_tickets_remote(self):
        transported_ticket_ids = self.mapped('transported_ticket_ids')
        tickets = self.env['ticket.pro'].search([])
        ticket_to_transport = []
        for ticket in tickets:
            if ticket.id not in transported_ticket_ids.ids:
                ticket_to_transport.append(ticket)
                self.transported_ticket_ids = [(4,ticket.id)]
        for future_tk in ticket_to_transport:
            try:
                 self.create_remote_tickets(future_tk)
            except:
                self.transported_ticket_ids = [(3, future_tk.id)]
                pass
